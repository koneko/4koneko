...
class Game {
	...
	developerUpdate() {
		...
		if (this.developer.devTextObj != null)
			this.developer.devTextObj.text = `MS: ${this.ms}\nOffset: ${globalOffset}\nInitial Offset: ${globalInitialStartTime}\nPoints: ${this.points}\nDifficulty: ${globalDifficulty}\nArrows: ${this.arrows.length}\nLNCounter: ${longNoteDevelopmentCounter}`;
		else
			this.developer.devTextObj = renderer.createText(0, 0, ``, {
				fontSize: 24,
				fill: 0xffffff,
				align: "center",
				stroke: 0x000000,
		...
		if (this.developer.judgementsObj != null)
			this.developer.judgementsObj.text = `${this.getAccuracy()}%\nMarvelous: ${
			...
			this.developer.judgementsObj = renderer.createText(
				0,
				screenHeight / 3,
	...
	pause() {
		...
		this.modal(`
        ${
			localStorage.extraOptions == "true"
				? `<p style="font-size: 10px;">MS: ${game.ms}</p>`
				: ""
		}
        ${
			localStorage.extraOptions == "true"
        ...
        ${
			localStorage.extraOptions == "true"
				? `<p style="cursor: pointer; color: red; font-size: 12px; margin: 0" onclick="game.autoPlay()">AutoPlay</p>`
				: ""
		}
        ${
			localStorage.extraOptions == "true"
				? `<p style="cursor: pointer; color: green; font-size: 12px; margin: 0" onclick="game.disableOffset()">NoOffset</p>`
				: ""
		}
        ${
			localStorage.extraOptions == "true"
				? `<p style="cursor: pointer; color: blue; font-size: 12px; margin: 0"onclick="game.disassembler()">Disassembler</p>`
				: ""
		}
        ...
        <p style="cursor: pointer;" onclick="window.location.href = '../'">Quit</p>
        `);
	...
	disableOffset() {
		if (params.get("disableGlobalOffset") == "true")
			window.location = "./rwk/?file=" + file + "&id=" + id;
		else
			window.location =
				"./rwk/?file=" +
				file +
	...
	autoPlay() {
		if (params.get("autoPlay") == "true")
			...
			window.location =
				"./rwk/?file=" + file + "&id=" + id + "&autoPlay=true";
	}
	update() {
		this.updateArrows();
	}
	updateArrows() {
		...
		this.notes.forEach((note) => {
			if (note.seconds <= this.ms && note.consumed == false) {
				...
				if (
					note.endTime != null &&
					localStorage.experimentalMode == "true"
				)
					arrow = new LongNote(
						game,
						note.lane,
	...
	endGame() {
		...
		let gradetext = renderer.createText(
			screenWidth / 2,
			screenHeight / 2 - 60,
			`${letter}`,
			{
		...
		let accuracytext = renderer.createText(
			screenWidth / 2,
			screenHeight / 2,
			`Accuracy: ${accuracy}%\nPress Esc to leave!`,
			{
	...
	getAccuracy() {
		...
		let accuracy =
			(this.judgements.marvelous * 100 +
				this.judgements.perfect * 98.25 +
				this.judgements.great * 65 +
				this.judgements.good * 25 +
				this.judgements.okay * -100 +
				this.judgements.miss * -50) /
			total;
	...
	getLetterAndColor(accuracy) {
		if (accuracy == 100) return { letter: "X", color: 0xffffff };
			...
			return { letter: "S", color: 0xffffff };
		else if (accuracy > 90 && accuracy < 95)
			return { letter: "A", color: 0x00ff00 };
		else if (accuracy > 80 && accuracy < 90)
			return { letter: "B", color: 0x0000ff };
		else if (accuracy > 70 && accuracy < 80)
			return { letter: "C", color: 0x1fc0d1 };
		else if (accuracy > 60 && accuracy < 70)
			return { letter: "D", color: 0xba2224 };
	...
	createLanes() {
		for (let i = 0; i < this.lanes.length; i++) {
			...
			this.lanes[i].lane = renderer.createLane(
				lane.x,
				lane.y,
				lane.color
			);
	...
	createJudgementText(input, color) {
		if (this.text != null) {
			...
			if (input == "Marvelous")
				this.text.style.fontFamily = "Roboto Condensed";
	...
	gradeArrow(arrow, different) {
		...
		if (this.comboText == null)
			this.comboText = renderer.createText(
				screenWidth / 2,
				screenHeight * 0.45,
				this.combo,
				{
